...
function updateAuthUI(){
  ...
  if (CURRENT_USER){
    ...
    who.textContent = `Hello, ${CURRENT_USER}`;
...
async function handleSignup(e){
  ...
  document.getElementById("authMsg").textContent = res.success ? "Signup successful!" : (res.error||"Signup failed");
  if (res.success){
    document.getElementById("authModal").close();
    updateAuthUI();
    // after sign-in, render everything from cloud/local
    populateYearSelect();
    renderTasks(); renderDashboard(); renderGradeBars(); renderYearBreakdown(); renderReminders(); renderJournal();
  }
}
...
async function handleLogin(e){
  ...
  document.getElementById("authMsg").textContent = res.success ? "Login successful!" : (res.error||"Login failed");
  if (res.success){
    document.getElementById("authModal").close();
    updateAuthUI();
    populateYearSelect();
